...
func (l *loggerMiddleware) Serve(ctx *iris.Context) {
	...
	if status == "200" || status == "201" {
		status = colors.Color(status, colors.BRIGHTGREEN)
	} else if status == "404" || status == "500" || status == "403" || status == "401" {
		status = colors.Color(status, colors.BRIGHTRED)
	}

	//finally print the logs
	if method == "GET" {
		mapLock.RLock()
		l.printf("%s %s - %s | %v | %4v | %s | %s", getText, timed, date, status, latency, ip, path)
		mapLock.RUnlock()
	} else if method == "POST" {
		mapLock.RLock()
		l.printf("%s %s - %s | %v | %4v | %s | %s", postText, timed, date, status, latency, ip, path)
		mapLock.RUnlock()
	} else if method == "PUT" {
		mapLock.RLock()
		l.printf("%s %s - %s | %v | %4v | %s | %s", putText, timed, date, status, latency, ip, path)
		mapLock.RUnlock()
	} else if method == "HEAD" {
		mapLock.RLock()
		l.printf("%s %s - %s | %v | %4v | %s | %s", headText, timed, date, status, latency, ip, path)
		mapLock.RUnlock()
	} else if method == "DELETE" {
		mapLock.RLock()
		l.printf("%s %s - %s | %v | %4v | %s | %s", delText, timed, date, status, latency, ip, path)
		mapLock.RUnlock()
	} else {
		mapLock.RLock()
		l.printf("%s - %s %v %4v %s %s %s", timed, date, status, latency, ip, method, path)
		mapLock.RUnlock()
	}
	ctx.Next()
}
...
func (l *loggerMiddleware2) Serve2(ctx *iris.Context) {
	...
	endTime = time.Now()
	timed = rightPad2Len(endTime.Format("15:04:05.00000"), "0", 15)
	date = endTime.Format("02/01/2006")
	latency = endTime.Sub(startTime).String()
	parts := strings.Split(latency, ".")
	if len(parts) == 2 {
